(function() {
  ...
  class FlowiseStatusIndicator {
    constructor(spanElement, serviceName, pollingInterval = 30 * 1000) {
      ...
      this.interval = pollingInterval;
      this._keepRunning = false;
      this.url = 'https://flowise.power.tec.br/ping';
    }
    ...
    async updateOnce() {
      try {
        const statusData = await this.fetchFlowiseStatus();
        ...
        if (statusData.status === 'ok') {
          this.container.className = 'status-indicator ok';
          this.container.dataset.tooltip = 
            `${this.serviceName}: ${statusData.message}\nResposta: ${statusData.response || 'pong'}\nAtualizado em: ${updatedAt}`;
        } else {
          this.container.className = 'status-indicator critical';
          this.container.dataset.tooltip = 
        ...
        this.container.className = 'status-indicator critical';
        this.container.dataset.tooltip = 
          `${this.serviceName}: Erro ao verificar status\nErro: ${err.message}\nAtualizado em: ${new Date().toLocaleString('pt-BR')}`;
      }
    }
