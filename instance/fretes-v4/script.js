document.addEventListener('DOMContentLoaded', () => {
    ...
    // Função para obter destinos e distâncias atuais
    const getDestinoAtual = () => {
        const select = document.getElementById('destino-select');
        const destinosSelecionados = Array.from(select.selectedOptions).map(option => option.value);
        
        // Se nenhum destino estiver selecionado, usar todos
        if (destinosSelecionados.length === 0) {
            destinosSelecionados.push(...Array.from(select.options).map(option => option.value));
        }
        ...
        // Se há distância manual, usar para todos os destinos
        if (isManual) {
            ...
            return { 
                ...
                distancia: distancia, 
                isManual: true,
                destinosComDistancia: destinosSelecionados.map(destino => ({ destino, distancia }))
            };
        }
        
        // Usar distâncias individuais de cada destino
        const destinosComDistancia = destinosSelecionados.map(destino => ({
            destino,
            distancia: DESTINOS[destino] || 25.0
        }));
        
        // Calcular distância média para exibição
        const distanciaMedia = destinosComDistancia.reduce((sum, item) => sum + item.distancia, 0) / destinosComDistancia.length;
        
        return { 
            destinos: destinosSelecionados, 
            distancia: distanciaMedia, 
            isManual: false,
            destinosComDistancia: destinosComDistancia
        };
    };
    ...
    const atualizarCalculosCards = () => {
        ...
        const { distancia, isManual, destinos } = getDestinoAtual();
        
        // Atualizar distância exibida
        document.getElementById('distancia-atual').textContent = `${distancia.toFixed(1)} km`;
        ...
        // Mostrar quantidade de destinos selecionados
        const destinosInfo = destinos.length === 1 ? destinos[0] : `${destinos.length} destinos selecionados`;
    ...
    const renderSimuladorFrete = (data) => {
        const { destinos } = getDestinoAtual();
        const dadosDestino = data.filter(row => destinos.includes(row[COLUMN_MAPPING.cliente]));
        
        if (dadosDestino.length === 0) {
            document.getElementById('frete-calculation').innerHTML = '<p>Nenhum dado encontrado para os destinos selecionados.</p>';
            return;
        }
        ...
        semanasOrdenadas.forEach(([semana, dados]) => {
            ...
            html += `<tr>
                ...
                <td>${formatCurrency(frete.valorFrete + frete.valorAdValorem)}</td>
            </tr>`;
        });
        ...
        html += `<p><strong>Total Geral (Frete + Ad Valorem):</strong> ${formatCurrency(totalFrete + totalAdValorem)}</p>`;
    ...
    const setupPriceListeners = () => {
        ...
        // Botão para salvar distância para os destinos selecionados
        document.getElementById('salvar-distancia').addEventListener('click', () => {
            const distanciaManual = document.getElementById('distancia-manual').value;
            const destinosSelecionados = Array.from(document.getElementById('destino-select').selectedOptions).map(option => option.value);
            
            if (distanciaManual && destinosSelecionados.length > 0) {
                const distancia = parseFloat(distanciaManual);
                
                // Salvar distância para todos os destinos selecionados
                destinosSelecionados.forEach(destino => {
                    DESTINOS[destino] = distancia;
                });
                
                // Atualizar o texto das opções no dropdown
                const select = document.getElementById('destino-select');
                destinosSelecionados.forEach(destino => {
                    const option = select.querySelector(`option[value="${destino}"]`);
                    if (option) {
                        option.textContent = `${destino} (${distanciaManual} km)`;
                    }
                });
                ...
                if (destinosSelecionados.length === 1) {
                    alert(`Distância de ${distanciaManual} km salva para ${destinosSelecionados[0]}`);
                } else {
                    alert(`Distância de ${distanciaManual} km salva para ${destinosSelecionados.length} destinos`);
                }
            } else {
                alert('Por favor, selecione pelo menos um destino e digite uma distância.');
            }
        });
        ...
        // Botão para selecionar todos os destinos
        document.getElementById('selecionar-todos').addEventListener('click', () => {
            const select = document.getElementById('destino-select');
            Array.from(select.options).forEach(option => {
                option.selected = true;
            });
        ...
        // Botão para limpar seleção de destinos
    ...
    const updateKPIs = (data) => {
        ...
        // Filtrar dados pelos destinos selecionados
        const dadosDestino = data.filter(item => destinos.includes(item[COLUMN_MAPPING.cliente]));
