document.addEventListener('DOMContentLoaded', () => {
    ...
    const carregarDestinos = (data) => {
        ...
        // Selecionar o primeiro destino por padrão
        if (destinosUnicos.length > 0) {
            select.value = destinosUnicos[0];
    ...
    const renderSimuladorFrete = (data) => {
        ...
        const dadosDestino = data.filter(row => row[COLUMN_MAPPING.cliente] === destino);
        ...
        html += '<thead><tr><th>Semana</th><th>Peso Mercadoria (kg)</th><th>Valor Mercadoria (R$)</th><th>Veículo</th><th>Cálculo Frete</th><th>Valor Frete (R$)</th><th>Total (R$)</th></tr></thead>';
        html += '<tbody>';
        ...
        semanasOrdenadas.forEach(([semana, dados]) => {
            const frete = calcularFrete(dados.peso);
            ...
            html += `<tr>
                ...
                <td>${formatCurrency(dados.valor + frete.valorFrete)}</td>
            </tr>`;
        });
        ...
        html += `<p><strong>Total Geral:</strong> ${formatCurrency(totalMercadoria + totalFrete)}</p>`;
    ...
    const setupPriceListeners = () => {
        ...
        document.getElementById('preco-pequeno').addEventListener('input', atualizarCalculosCards);
        document.getElementById('preco-medio').addEventListener('input', atualizarCalculosCards);
        document.getElementById('preco-grande').addEventListener('input', atualizarCalculosCards);
        ...
        document.getElementById('salvar-distancia').addEventListener('click', () => {
            ...
            if (distanciaManual && destinoAtual) {
