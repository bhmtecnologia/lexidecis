...
export async function renderConsultaCnpj() {
      ...
      <div class="page-title">
        <div class="row">
          <div class="col-sm-6 col-12">
            ...
            <p class="mb-0 text-title-gray">Digite o CNPJ e clique em "Consultar".</p>
          </div>
          <div class="col-sm-6 col-12">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active">CNPJ</li>
            </ol>
          </div>
        </div>
      </div>
                ...
                <div class="row align-items-end">
                  <div class="col-md-6">
                    <div class="form-group">
                      <label for="cnpj">CNPJ: <span id="cnpjFeedback" class="ml-2"></span></label>
  ...
  // Cache de referências dos elementos do DOM
  ...
  // Função para validar o CNPJ (verifica o padrão 00.000.000/0000-00)
  const validarCnpj = value => /^\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}$/.test(value);

  // Função para definir o estado de validação utilizando classList.toggle
  const setValidationState = (isValid) => {
    inputCnpj.classList.toggle('is-valid', isValid);
    inputCnpj.classList.toggle('is-invalid', !isValid);
    cnpjFeedback.textContent = isValid ? "válido" : "inválido";
    cnpjFeedback.classList.toggle('text-success', isValid);
    cnpjFeedback.classList.toggle('text-danger', !isValid);
  };

  // Aplica a máscara e validação em tempo real no campo CNPJ
  inputCnpj.addEventListener('input', (e) => {
    ...
    if (value.length === 18) {
      setValidationState(validarCnpj(value));
    } else {
      inputCnpj.classList.remove('is-valid', 'is-invalid');
      cnpjFeedback.textContent = "";
      cnpjFeedback.classList.remove('text-success', 'text-danger');
    }
  });
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // Desabilita botões e exibe o spinner
    [btnConsultar, btnDownload, btnDownloadCSV, btnDownloadDOCX, btnPrint].forEach(btn => btn.disabled = true);
    resultadoConteudo.innerHTML = "";
    spinner.style.display = "block";

    // Obter o token JWT via AuthService
    let token;
    ...
    const url = `https://n8n.bhm.tec.br/webhook/murillo`;
    ...
    try {
      ...
      resultadoConteudo.innerHTML = `
        <div class="alert alert-danger">
          Erro ao consultar o CNPJ: ${error.message}
        </div>
      `;
    } finally {
  ...
  function downloadPdf() {
    const filename = `relatorio_cnpj_${gerarTimestamp()}.pdf`;
    const opt = {
      margin: 1,
      filename: filename,
    ...
    html2pdf().set(opt).from(resultadoConteudo).save();
  }
  ...
  function downloadCsv() {
    ...
    const filename = `relatorio_cnpj_${gerarTimestamp()}.csv`;
  ...
  function downloadDocx() {
    ...
    const filename = `relatorio_cnpj_${gerarTimestamp()}.doc`;
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = filename;
...
registerRoute('#consulta-cnpj', renderConsultaCnpj);