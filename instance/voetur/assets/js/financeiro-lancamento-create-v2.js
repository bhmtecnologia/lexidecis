...
function handleError(error, context = "Erro") {
  ...
  // Agora, o campo Filial também foi adaptado para usar Select2.
  function resetFormFields() {
    ...
    if (window.filiaisData && window.filiaisData.length === 1) {
      ...
      // Para bloquear a alteração, definimos o atributo readonly (ou disabled)
    ...
    if (window.projetosData && window.projetosData.length === 1) {
      ...
      projetoSelect.value = proj.nome;
      projetoSelect.setAttribute("readonly", "true");
    } else {
  ...
  export async function renderFinanceiroLancamentoCreateV2() {
    ...
    content.innerHTML = `
          ...
          <div class="card" style="border: 1px solid var(--border-color); background-color: var(--white); color: var(--black);">
            <div class="card-body">
              <form id="lancamentoForm">
                ...
                <!-- Campo: Centro de Custo (usando Select2) -->
                <div class="mb-3">
      ...
      // Inicializa o Select2 para os campos Filial, Fornecedor, Conta Financeira, Centro de Custo e Projeto
      if (window.$ && $.fn.select2) {
        $('#filialSelect').select2({
      ...
      // Validações para cada campo usando o Select2
      // Filial
      const filialSelect = document.getElementById("filialSelect");
      ...
      projetoSelectField.addEventListener("change", function () {
        const inputValue = this.value.trim().toLowerCase();
        // Campo opcional – somente valida se houver valor selecionado
      ...
      // Outras validações não ligadas ao Select2
      ...
      // Carrega os dados através do AuthService.onAuthChange para evitar duplicações
      AuthService.onAuthChange((user) => {
        if (user) {
          ...
          (async () => {
            try {
              // Filiais – Preenche o select para uso do Select2
              const filialSelect = document.getElementById("filialSelect");
              ...
              if (window.filiaisData.length === 1) {
                const fil = window.filiaisData[0];
                filialSelect.innerHTML = `<option value="${fil.nome}">${fil.nome}</option>`;
                filialSelect.setAttribute("readonly", "true");
              } else {
          ...
          (async () => {
            try {
              // Centros de Custo – Preenche o select para uso do Select2
              const centroSelect = document.getElementById("centroCustoSelect");
              ...
              if (window.centrosData.length === 1) {
                const centro = window.centrosData[0];
                centroSelect.innerHTML = `<option value="${centro.nome}">${centro.nome}</option>`;
                centroSelect.setAttribute("readonly", "true");
              } else {
          ...
          (async () => {
            try {
              // Projetos – Preenche o select para uso do Select2
              const projetoSelect = document.getElementById("projetoSelect");
              ...
              if (window.projetosData.length === 1) {
                const proj = window.projetosData[0];
                projetoSelect.innerHTML = `<option value="${proj.nome}">${proj.nome}</option>`;
                projetoSelect.setAttribute("readonly", "true");
              } else {
          ...
          (async () => {
            try {
              // Fornecedores – Preenche o select para uso do Select2
