...
export async function renderVtcFinanceiroGestor() {
  ...
  document.getElementById('btnLaunch').addEventListener('click', () => {
    window.location.hash = '#vtc-financeiro-lancamento-create-v6';
  });
  // demais handlers iguais...

  // Handler para abrir modal de edição com estrutura estática (igual create-v6)
  $('#lancamentosTable tbody').on('click', 'button[data-action="edit"]', function() {
    ...
    if (lanc) {
      ...
      // --- Select2 initialization logic as in create-v6 ---
      // Filial
      ...
      // Manual-toggle buttons: attach click handler as in create-v6
