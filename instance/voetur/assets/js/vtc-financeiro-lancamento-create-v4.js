/**
* @file vtc-financeiro-lancamento-create-v4.js
* @description Versão v4 da página de "Financeiro - Lançamento Create", permite criação de lançamentos via formulário
* e envia os dados para a API v4.
 ...
 *
* API de lançamento utilizada (v4): https://n8n.power.tec.br/webhook-test/voetur/v4/lancamentos
 */
...
export async function renderVtcFinanceiroLancamentoCreateV4() {
  ...
  content.innerHTML = `
      ...
      <div class="page-title" style="padding: 1rem;">
        <div class="row">
          <div class="col-sm-6 col-12">
            <h2 style="color: var(--black);">Financeiro - Lançamento Create V4</h2>
            <p class="mb-0" style="color: var(--black);">Crie e gerencie novos lançamentos de pagamento (v4)</p>
          </div>
          <div class="col-sm-6 col-12">
            <ol class="breadcrumb" style="color: var(--theme-default);">
              ...
              <li class="breadcrumb-item active" style="color: var(--theme-default);">Lançamento Create V4</li>
            </ol>
          </div>
        </div>
      </div>
    ...
    <div class="mb-3">
      ...
      <div id="dropZone" class="dropzone">
        ...
        <input type="file" id="arquivoClassify" accept="image/png,image/jpeg" multiple class="d-none">
      </div>
        ...
        <div class="card" style="border:1px solid var(--border-color);background:var(--white);color:var(--black);">
          <div class="card-body">
            <form id="lancamentoForm">
              ...
              <button type="submit" class="btn btn-primary">Criar Lançamento v4</button>
  ...
  arquivoClassify.addEventListener("change", async (e) => {
    ...
    try {
      ...
      // Realiza upload e classificação para todos arquivos em paralelo
      const resultados = await Promise.all(
        Array.from(files).map(file => uploadArquivo(AuthService, file, { classify: true }))
      );
      // Recarrega fornecedores uma vez (antes do loop)
      try {
        window.fornecedoresData = await listFornecedores(AuthService);
      } catch (err) {
        console.error("Erro ao recarregar fornecedores:", err);
      }
      // Para cada documento classificado, dispara o lançamento automático
      for (let idx = 0; idx < resultados.length; idx++) {
        const info = resultados[idx];
        ...
        if (info.tipo_documento === "Nota Fiscal") {
          // Preenche campos da nota fiscal com checagem de existência (não exibe form, só auto-cria)
          // Preenche e bloqueia campos, mas não mostra form-section
          // Preenche fornecedor e filial para payload
          // Auto-cria lançamento para Nota Fiscal com status Importado por lote
          try {
            const payloadAuto = {
            ...
            // Projeto: se existir no info (ex: info.projeto) ou único
            if (window.projetosData && window.projetosData.length === 1) {
              const p = window.projetosData[0];
              payloadAuto.projeto_id = p.id || p.uuid;
              payloadAuto.projeto_nome = p.nome;
            }
            // Itens da Nota Fiscal
            if (info.itens && Array.isArray(info.itens)) {
              payloadAuto.itens = info.itens.map(item => ({
                ...
                valor_unitario: (() => {
                  ...
                  const num = raw && typeof raw === "string"
                    ? Number(raw.replace(/\./g, "").replace(",", "."))
                    : Number(raw) || 0;
                  return num;
                })()
            ...
            document.querySelectorAll('.parcela-item').forEach(item => {
              ...
              if (dateEl && valorEl && dateEl.value) {
                const rawVal = valorEl.value.replace(/\./g, '').replace(',', '.');
      ...
      // Após processar todos, limpa seleção e retorna ao gestor
      arquivoClassify.value = "";
  ...
  AuthService.onAuthChange(user => {
    ...
    .then(([filiaisData, centrosData, projetosData, fornecedoresData]) => {
      ...
      // Define forma de pagamento padrão como Depósito
      const formaEl = document.getElementById("formaPagamento");
      if (formaEl) {
        formaEl.value = "Depósito";
        // Se estiver usando Select2 ou form styling, pode ser necessário disparar change
        if (window.$ && $.fn.select2) {
  ...
  lancForm.addEventListener("submit", async e => {
    ...
    */
    ...
    try {
      ...
      // **Aqui** chame createLancamento para o endpoint v4
      const result = await createLancamento(AuthService, payload, { apiVersion: "v4" });
      ...
      if (pageTitleEl) pageTitleEl.textContent = "Financeiro - Lançamento Create V4";
    } catch (err) {
...
// Registra a rota v4
registerRoute("#vtc-financeiro-lancamento-create-v4", renderVtcFinanceiroLancamentoCreateV4);
