/**
 * @file vtc-financeiro-lancamento-create-v5.js
 * @description Versão v5 da página de "Financeiro - Lançamento Create", permite criação de lançamentos via formulário e envia os dados para a API v5.
...
export async function renderFinanceiroLancamentoCreateV5() {
  ...
  function buildReview() {
    ...
    // Parcelas
    const parcelas = Array.from(document.querySelectorAll('#parcelasTable tbody tr')).map((tr, i) => {
      const val = tr.querySelector('input[name="parcelaValor[]"]')?.value;
      const dt  = tr.querySelector('input[name="parcelaDataVenc[]"]')?.value;
      return { dt, val };
    });
    if (parcelas.length) {
      const header = document.createElement("h5");
      header.textContent = "Parcelas";
      review.appendChild(header);
      parcelas.forEach((p, i) => {
        const div = document.createElement("div");
        div.className = "mb-1";
        div.innerHTML = `Parcela ${i+1}: <strong>Valor</strong> ${p.val}, <strong>Vencimento</strong> ${p.dt}`;
        review.appendChild(div);
      });
    }
    // Itens
    const itens = Array.from(document.querySelectorAll('#itensTable tbody tr')).map((tr, i) => {
      const desc = tr.querySelector('input[name="itemDescricao[]"]')?.value;
      const qty  = tr.querySelector('input[name="itemQuantidade[]"]')?.value;
      const vu   = tr.querySelector('input[name="itemValorUnitario[]"]')?.value;
      return { desc, qty, vu };
    });
    if (itens.length) {
      const header = document.createElement("h5");
      header.textContent = "Itens";
      review.appendChild(header);
      itens.forEach((it, i) => {
        const div = document.createElement("div");
        div.className = "mb-1";
        div.innerHTML = `Item ${i+1}: <strong>Descrição</strong> ${it.desc}, <strong>Qtd</strong> ${it.qty}, <strong>Valor Unit.</strong> ${it.vu}`;
        review.appendChild(div);
      });
    }
  }
  ...
  lancForm.addEventListener("submit", async e => {
    ...
    try {
      ...
      console.log("Payload v5:", JSON.stringify(payload, null, 2));
      // **Aqui** chame createLancamento para o endpoint v5
      const result = await createLancamento(AuthService, payload, { apiVersion: "v5" });
      ...
      if (pageTitleEl) pageTitleEl.textContent = "Financeiro - Lançamento Create V5";
    } catch (err) {
      formError.innerHTML = `Erro ao criar lançamento: ${handleError(err, "CreateLancamentoV5")}`;
    } finally {
      hideLoadingOverlay();
      btn.disabled = false;
      btn.innerHTML = original;
...
// Registra a rota v5
registerRoute("#vtc-financeiro-lancamento-create-v5", renderFinanceiroLancamentoCreateV5);
